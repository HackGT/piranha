...
export const canCancel = (parent: any, args: any, context: { user: User }) => {
    return isExec(context.user);
}
...
export const isAuthenticatedRule = rule({ cache: 'contextual' })(
    async (parent: any, args: any, context: { user: User }) => {
        return context.user !== null && context.user.accessLevel != AccessLevel.NONE
    },
);
