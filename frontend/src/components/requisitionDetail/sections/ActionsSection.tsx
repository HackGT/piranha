...
const ActionsSection: React.FC<RequisitionSectionProps> = (props) => {
  ...
  return (
    <>
      ...
      <Tooltip title={!data.canEdit && "You must be a project lead or exec member to edit a requisition after submission."}>
        <Button className="action-button" onClick={handleEdit} disabled={!data.canEdit}>Edit</Button>
      </Tooltip>
